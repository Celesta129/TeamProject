...
void CGameFramework::CreateDirect3DDevice()
{
	...
	if (!pd3dAdapter)	// 생성할수 없었다면 WARP 디바이스 생성
	{
	...
	// 지원하는 다중 샘플링 품질 수준 확인

	m_bMsaa4xEnable = (m_nMsaa4xQualityLevels > 1) ? true : false;
	// 다중샘플링 품질 수준이 1보다 높으면 다중샘플링 활성화.
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	// 명령리스트는 생성되었을 시 열린(open)상태이므로 닫아준다.
}
...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	...
	d3dDescriptorHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;		// RenderTarget View
	d3dDescriptorHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
	d3dDescriptorHeapDesc.NodeMask = 0;
	HRESULT hResult = m_pd3dDevice->CreateDescriptorHeap(&d3dDescriptorHeapDesc, __uuidof(ID3D12DescriptorHeap), (void**)&m_pd3dRtvDescriptorHeap);
	m_nRtvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	// 렌더타겟 서술자 힙 메모리 증가폭을 저장.
