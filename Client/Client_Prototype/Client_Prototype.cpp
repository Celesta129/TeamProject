...
void DrawButton(HDC hdc, int x, int y, int width, int height, HBITMAP &button) {
	HDC MemDC;
	HBITMAP OldBitmap;

	MemDC = CreateCompatibleDC(hdc);
	OldBitmap = (HBITMAP)SelectObject(MemDC, button);

	TransparentBlt(hdc, x, y, width, height, MemDC, 0, 0, width, height, RGB(255, 255, 255));
	SelectObject(MemDC, OldBitmap);
	DeleteDC(MemDC);
}
...
void process_packet(char* buf) {
	...
	case SC_WAITROOM:
	{
		sc_packet_waitroom p_waitroom;
		int playernum = p_waitroom.id;
		char playername[15];
		strcpy_s(playername, p_waitroom.name);
	...
	try {
		...
		if (GameFramework->m_pSocket->init()) {
			...
			while (1)
			{
				...
				if (Is_Host) {
					cout << "Type Start. if you want to start a game.";
					cin >> typing;
