...
D3D12_RASTERIZER_DESC CObjectShader::CreateRasterizerState()
{
...
D3D12_BLEND_DESC CObjectShader::CreateBlendState()
{
...
D3D12_DEPTH_STENCIL_DESC CObjectShader::CreateDepthStencilState()
{
...
void CObjectShader::CreatePSO(ID3D12Device * pd3dDevice, UINT nRenderTargets, int index)
{
...
void CObjectShader::CreateFrameResources(ID3D12Device * pd3dDevice)
{
...
void CObjectShader::CreateDescriptorHeaps(ID3D12Device * pd3dDevice)
{
...
void CObjectShader::CreateRootSignature(ID3D12Device * pd3dDevice)
{
...
void CObjectShader::CreateShader(ID3D12Device * pd3dDevice, ID3D12RootSignature * pd3dGraphicsRootSignature)
{
	ID3DBlob *pd3dVertexShaderBlob = NULL, *pd3dPixelShaderBlob = NULL;
	D3D12_GRAPHICS_PIPELINE_STATE_DESC d3dPipelineStateDesc;
	::ZeroMemory(&d3dPipelineStateDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
	d3dPipelineStateDesc.pRootSignature = pd3dGraphicsRootSignature;
	d3dPipelineStateDesc.VS = CreateVertexShader(&pd3dVertexShaderBlob, "PS");
	d3dPipelineStateDesc.PS = CreatePixelShader(&pd3dPixelShaderBlob, "VS");
...
void CObjectShader::CreateShaderVariables(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList)
{

}

void CObjectShader::CreateConstantBufferViews(ID3D12Device * pDevice)
{
...
void CObjectShader::UpdateMainPassCB(CCamera * pCamera)
{
...
void CObjectShader::OnPrepareRender(ID3D12GraphicsCommandList * pd3dCommandList)
{
...
void CObjectShader::Render(ID3D12GraphicsCommandList * pd3dCommandList, CCamera * pCamera, UINT64 nFenceValue)
{
	...
	for (UINT index = 0; index < m_vpObjects.size(); ++index)
	{
		...
		D3D12_GPU_VIRTUAL_ADDRESS whereView = m_CurrFrameResource->ObjectCB.get()->Resource()->GetGPUVirtualAddress() + cbvIndex;

		//pd3dCommandList->SetGraphicsRootConstantBufferView(1, whereView);
